...
def updateGraph(n):
    ...
    for edge in edges: # Update ma trận kề, vì arr start tại 0 nên phải -1 để tới đỉnh tương ứng
...
# In kết quả
for i in range(0,q):
    print("From vertex "+str(queries[i][0])+" to vertex " +str(queries[i][1]) +" : "+ str(Result[queries[i][0]-1,queries[i][1]-1]) +" ways")
